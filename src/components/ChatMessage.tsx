...
import { User, Package, Sparkles, Rocket, HelpCircle, Calendar, FileText, Gift, SkipForward } from 'lucide-react';
...
export const ChatMessage: React.FC<ChatMessageProps> = ({ message, onSelectOffer, onQuickReply }) => {
  ...
  // Render add-ons prompt with quick reply buttons
  if (message.type === 'addonsPrompt' && onQuickReply) {
    return (
      <motion.div
        initial={{ opacity: 0, y: 10 }}
        animate={{ opacity: 1, y: 0 }}
      ...
      >
        ...
        <div className="flex flex-col max-w-[80%]">
          <div className="px-5 py-3.5 rounded-2xl text-[15px] leading-relaxed shadow-sm bg-gradient-to-br from-purple-50 to-indigo-50 border border-purple-100 text-gray-800 rounded-tl-none">
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              <button 
                onClick={() => onQuickReply("Show me!")}
                className="inline-flex items-center gap-1.5 px-4 py-2 bg-purple-600 text-white rounded-full text-sm font-medium hover:bg-purple-700 transition-colors shadow-sm"
              >
                <Gift className="w-4 h-4" />
                Show me!
              </button>
              <button 
                onClick={() => onQuickReply("Skip for now")}
                className="inline-flex items-center gap-1.5 px-4 py-2 bg-gray-100 text-gray-600 rounded-full text-sm font-medium hover:bg-gray-200 transition-colors"
              >
